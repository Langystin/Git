...
```
3) **Скопировать URL репозитория на GitHub**. Чтобы это сделать нужно:
...
cd <имя репозитория>
```
2) **Добавляем изменненные файлы в индекс** с помощью команды `git add` указывая конкретный файл
```bash
git add README.md
```
3) **Создаем коммит с комментарием который опиcывает изменение**.
```bash
